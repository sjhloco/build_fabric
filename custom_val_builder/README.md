...
```yaml
cmds:
  - show ip ospf intefrace brief vrf all:
      dummy: dummy
```
...
1. **INPUT_DM:** Now that we know what we will get back from the device this can be manipulated into a data model for the attributes we want to validate. For example, if I want to ensure that the interfaces have the correct OSPF process, area and are up I would want an actual state dictionary for each interface in the following format:
...
As the data model is created in python is easier than Ansible to troubleshoot, you can print this out using the 'dm' tag.
...
Using the `ansible-playbook PB_val_builder.yml  -i hosts --tag tmpl` the rendered output (desired state) will be printed to screen as well as saved to *desired_state.yml*.
...
A compliance report missing error mean` that it is in the desired_state but not in the actual state. For example the below error means that *'10010'* is not in actual state. Be careful with string and integers, this was caused because needed to be a string in desired state
...
Strict has to be at the level where you want to enforce it. If an extra element is defined will get an 'extra' error in the compliance report
ok: [DC1-N9K-BORDER01] => {
    "result": {
        "complies": false,
        "show nve vni": {
